...
os.environ["CHIME_FRB_ACCESS_TOKEN"] = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyX2lkIjoicmRhcmxpbmdlciIsImV4cCI6MTc1Mjc2Nzk0NiwiaXNzIjoiZnJiLW1hc3RlciIsImlhdCI6MTc1Mjc2NjE0Nn0.lljXPKM_5SQjW-sV85ek8ZbQtLqsEk89Z8cZrAZMdA8"
os.environ["CHIME_FRB_REFRESH_TOKEN"] = "9978e7eedabd43377cd54568b1a6b1962a7092e6571db07e"
...
def frb_rerun_pipeline(events, dms, telescope="gbo"):
    ...
    dms: list
        Structure maximized DMs corresponding to each event ID given
    telescope: str
        Name of outrigger telescope
    
    Returns:
    ________
    fig: matplotlib.pyplot.figure, csv
        Diagnostic plots for each FRB including: original waterfall from singlebeam, new waterfall from new beamforming,  visual of gain, auto correlation SNR, csv containing SNR, fraction masked (Both in gain and in beamforming), event id, and date (soon to included cross correlation)
    ...
    for i, event_id in enumerate(events):
        ...
        try:
            ...
            toa=dt.timestamp()
        transit_time=find_files(output_file, f"/arc/projects/chime_frb/rdarlinger/n_squared_files", toa)
...
def just_make_gains(events, telescope="gbo"):
    """
    Makes gains just for the days of the FRBs
    ...
    ________
    filepath: str
        Filepath to the gain that was created

    """
    master = frb_master.FRBMaster()
    for i, event_id in enumerate(events):
        path = '/arc/projects/chime_frb/data/gbo/daily_gain_solutions/offline_gains/'
        if not os.path.isdir(path):
            print("Making new dir: ", path)
            os.mkdir(path)
        print("Getting information for {}".format(event_id))
        ...
        try:
            ...
            toa=dt.timestamp()
        src=get_best_gain_calibrator(toa, "gbo")
        print(src)
        transit_time=find_files(output_file, f"{path}n_squared_files", toa, src_str=src)
        flagged, filepath = get_gains_from_N2(f"/arc/projects/chime_frb/data/{telescope}/n_squared/{formatted_dt}_{telescope}_corr/", transit_times=transit_time, src_str=src, gains_output_dir=path, plot_output_dir="/arc/projects/chime_frb/rdarlinger/offline_gain_diagnoistic_plots/")
        return filepath
...
if __name__ == "__main__":
    just_make_gains([356507004])
    #frb_rerun_pipeline([366503638], [461.595690858361]) #350136130, 358105468, , 378287810, 383577603, 388211354, 397220423, 412590956, 424530814,432660091      1118.30737623314, 630.7783269532234, , 426.796157134772, 1088.30995456379, 787.642026481932, 349.32632456817106, 503.81586571046, 1025.96991261613,686.208237421834